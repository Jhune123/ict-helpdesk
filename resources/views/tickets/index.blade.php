...
<div class="p-6">
    ...
    {{-- FILTER & EXPORT SECTION (KEEPING YOUR EXISTING LOGIC) --}}
    <div class="flex flex-wrap items-center justify-between mb-4 gap-2">
        <form method="GET" class="flex flex-wrap items-end gap-4 w-full md:flex-nowrap">
            ...
            <div class="flex items-end gap-2">
                ...
                <button class="px-4 py-2 bg-blue-600 text-white rounded-lg shadow font-semibold hover:bg-blue-700">Filter</button>
                <a href="{{ route('tickets.index') }}" class="px-4 py-2 bg-gray-400 text-white rounded-lg shadow hover:bg-gray-500">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="overflow-x-auto bg-white shadow-md rounded-lg">
        <table class="min-w-full divide-y text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr class="{{ $ticket->status === 'Closed' ? 'bg-green-50 hover:bg-green-100' : ($ticket->status === 'Condemned' ? 'bg-red-50 text-red-900 hover:bg-red-100' : 'hover:bg-gray-50') }}">
                    ...
                    {{-- ✅ HARD-FIX FOR CATEGORY URL ERROR --}}
                    <td class="px-3 py-2">
                        @if($ticket->category && isset($ticket->category->id))
                            <a href="/categories/{{ $ticket->category->id }}" class="text-blue-600 hover:underline">
                                {{ $ticket->category->name }}
                            </a>
                        @else
                            <span class="text-gray-500">
                                {{ $ticket->category->name ?? ucfirst(str_replace('_', ' ', $ticket->form_type ?? 'Uncategorized')) }}
                            </span>
                        @endif
                    </td>
                    ...
                    <td class="px-3 py-2">
                        <span class="px-2 py-0.5 rounded text-[10px] uppercase font-bold {{ $ticket->priority === 'High' ? 'bg-red-200 text-red-800' : 'bg-blue-100 text-blue-800' }}">
                            {{ $ticket->priority ?? 'Normal' }}
                        </span>
                    </td>
                    <td class="px-3 py-2 font-semibold">
                        <span class="{{ $ticket->status === 'Closed' ? 'text-green-700' : '' }} {{ $ticket->status === 'Condemned' ? 'text-red-700' : '' }}">
                            {{ $ticket->status }}
                        </span>
                    </td>

                    <td class="px-3 py-2 text-center space-x-1 whitespace-nowrap">
                        <a href="{{ url('tickets/' . $ticket->id) }}" class="bg-blue-500 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">View</a>

                        <a href="{{ url('tickets/' . $ticket->id . '/job-order') }}" target="_blank" class="bg-indigo-500 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">Job Order</a>

                        @if(in_array($ticket->status, ['Closed', 'Condemned']))
                            @if(!$ticket->feedback)
                                <a href="{{ url('feedbacks/create/' . $ticket->id) }}" class="bg-pink-600 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">Feedback</a>
                            @else
                        ...
                        @role('admin|it_staff')
                            <a href="{{ url('tickets/' . $ticket->id . '/edit') }}" class="bg-yellow-500 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">Edit</a>
                            <form action="{{ url('tickets/' . $ticket->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button onclick="return confirm('Permanently delete this ticket?')" class="bg-red-600 text-white px-2 py-1 rounded text-[10px] font-bold uppercase shadow-sm">Delete</button>
                            </form>
                        @endrole
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="12" class="text-center py-10 text-gray-500 italic">No tickets found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
